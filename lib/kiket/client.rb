...
module Kiket
  class Client
    ...
    # Get raw binary response (for PDF downloads, etc.)
    def get_raw(path, params: {}, headers: {})
      response = raw_connection.get do |req|
        req.url path
    ...
    # Connection for raw binary responses (no JSON parsing)
    def raw_connection
      @raw_connection ||= Faraday.new(url: config.api_base_url) do |conn|
        conn.request :retry, max: 3, interval: 0.5, backoff_factor: 2
        conn.response :logger if config.verbose
    ...
    def handle_raw_response(response)
      ...
      when 500..599
        ...
        raise APIError.new("Unknown error", status: response.status, response_body: response.body)
      end
    end
  end
end
